...
class TolokaProcessingApi:
    ...
    def get_pool_completed_tasks(self, pool_id):
        ...
        df['winner_url'] = df.apply(self.get_winner_url, axis=1)
        df['loser_url'] = df.apply(self.get_loser_url, axis=1)
        return df
    ...
    def update_pool_params(self, pool_id, pool_params):
        req = requests.put("https://toloka.yandex.ru//api/v1/pools/{}".format(pool_id), headers = self.headers, json=pool_params)
        assert req.ok
